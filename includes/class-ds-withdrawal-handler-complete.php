...
class DS_Withdrawal_Handler_Complete {
    ...
    public function init_withdrawal_handler() {
        $this->form_id = function_exists('get_field') ? get_field('withdrawal_form_id', 'option') : null;
    ...
    public function populate_current_balance($form) {
        if (!is_user_logged_in() || !class_exists('DS_Credit_Manager')) {
            return $form;
        }
        ...
        foreach ($form['fields'] as &$field) {
            if (isset($field->inputName) && $field->inputName == 'saldo_atual') {
    ...
    public function validate_withdrawal($validation_result) {
        ...
        $amount_field_id = $this->get_field_id_by_label('Valor do Saque', $form);
        $method_field_id = $this->get_field_id_by_label('Método de Pagamento', $form);
        $user_id_field_id = $this->get_field_id_by_label('ID do Usuário', $form);
        $balance_field_id = $this->get_field_id_by_label('Saldo Atual', $form);
    ...
    public function process_withdrawal_request($entry, $form) {
        ...
        $amount_field_id = $this->get_field_id_by_label('Valor do Saque', $form);
        $method_field_id = $this->get_field_id_by_label('Método de Pagamento', $form);
        $notes_field_id = $this->get_field_id_by_label('Observações', $form);
    ...
    private function send_user_withdrawal_notification($entry, $status, $reason = '') {
        ...
        if (!$user_phone || !$user_data) {
            GFAPI::add_note($entry['id'], 0, 'Sistema', 'ERRO: Telefone ou dados do usuário não encontrados.');
            return;
        }
        ...
        if (class_exists('WhatsApp_Connector')) {
            if ($status === self::STATUS_APPROVED) {
                $queued = WhatsApp_Connector::send_template('withdrawal_approved', $user_phone, [
                    'Nome_Usuario' => $user_name,
                    'quantia_creditos' => $amount
                ], 'critical');
            } elseif ($status === self::STATUS_REJECTED) {
                $queued = WhatsApp_Connector::send_template('withdrawal_rejected', $user_phone, [
                    'Nome_Usuario' => $user_name,
                    'quantia_creditos' => $amount
                ], 'critical');
            }
